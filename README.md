...
* `createShared(view, pluginId, apiPrefix)`: a per view helper bag with `escapeHtml`, `setStatus`, `getConfig`/`saveConfig`/`apiRequest`, `formatSize`/`formatDuration`/`formatDate`, `badge`, `statusBadge`, `emptySection`, `createSearchableComboBox`, `createDebouncedSearch`, `createUserSelector`/`createUserMultiSelector`, and `pollTaskProgress`.
...
```xml
<PackageReference Include="JPKribs.Jellyfin.Base" Version="2026.6.21" />
```
